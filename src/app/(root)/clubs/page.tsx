...
import { getClubCategories, getFeaturedClub } from './action';
...
export default function VenuesPage() {
  ...
  const { data:categories, error, isLoading } = useSWR<RestaurantCategory[]>(
    ...
    async () => {
      try {
        const restaurantsCategory = await getClubCategories();
 ...
 const { data:venues } =useSWR<Restaurant[]>(
    ...
    async () => {
      try {
        const restaurants = await getFeaturedClub();
    ...
    <main className="min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-4 py-20 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <span className="text-[#37b7ab] font-semibold mb-2 block">            {translations["categoriesSectionSubtitle"]}</span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {venues?.map(venue => (
              <Link
                key={venue.id}
                href={`/clubs/${venue.category.slug}/${venue.slug}`}
